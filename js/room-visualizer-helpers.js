...
export function buildCompositePlan({ catalog, selection, manifest, space = "living", activeCategory = null }) {
  ...
  // 바닥/벽/천장/문 편집 중에는 항상 방 전체(선택된 4부위)를 미리보기에 유지한다.
  // (부위별로 일부만 보이면, 예: 벽지 고른 뒤 천장으로 넘어갈 때 벽지가 사라져 "초기화"처럼 보임)
  const layerPreviewItems = ["floor", "wall", "ceiling", "door"].includes(activeCategory)
  ...
  // 부위 조립 미리보기 베이스 = 선택한 천장의 풀룸 렌더.
  // (정적 평천장 베이스 위에 단내림/우물천장 같은 다른 천장 레이어를 얹으면, 천장-벽 경계의
  //  소핏 부분은 천장 렌더, 그 아래 벽은 평천장 베이스라 서로 다른 렌더가 만나 톤 단차/이중톤이 생김)
  let layeredBase = room.base ?? null;
  if (layerPreviewItems) {
    const ceilingLayer = (rawRoom.layers ?? []).find((layer) => layer.item === "ceiling");
    const ceilOpt = lineOf(selection, space, "ceiling")?.optionCode;
    const ceilCfg = ceilOpt != null ? (ceilingLayer?.assets ?? {})[ceilOpt] : null;
    const ceilAsset = resolveLayerAsset(ceilCfg, { selection, space });
    const ceilSrc = typeof ceilAsset === "string" ? ceilAsset : ceilAsset?.src;
    if (ceilSrc) layeredBase = ceilSrc;
  }
  ...
  return { base: layeredBase, size: room.size ?? null, layers, pending };
}
