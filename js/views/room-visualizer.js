...
const VISUAL_ASSET_VERSION = "0622-wall-expansion-v4";
...
export function buildRoomVisualizer({ catalog, selection, manifest, space = "living", activeCategory = null }) {
  ...
  if (!plan.base) {
    ...
    const layerNodes = plan.layers.map((layer) => {
      const visualLayer = { ...layer };
      if (visualLayer.src) visualLayer.src = visualAssetUrl(visualLayer.src);
      if (visualLayer.zone) visualLayer.zone = visualAssetUrl(visualLayer.zone);
      return el("div", { style: layerToStyle(visualLayer) });
    });
    const stage = el("div",
